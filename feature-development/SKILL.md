...
description: Implement new features, user stories, enhancements, commands, views, services, endpoints, and workflow extensions in any codebase, using the repository's existing architecture, patterns, tests, and build conventions. Use whenever the task is to add, build, implement, extend, or support new behavior, even when the word "feature" is not used. Language-agnostic; detects the stack and loads the matching stack skill (dotnet-wpf, blazor) for conventions and validation commands, and hands the plan and the finished diff to review-gate for independent verification. Do not use when the primary task is diagnosing existing incorrect behavior. As a tie-breaker, if a request mixes a change with any symptom of existing behavior being wrong, broken, odd, or not updating, bug-fixing takes precedence; start there.
---
...
Extract the desired behavior, affected user flow or component, explicit constraints, and likely non-functional requirements (performance, concurrency, persistence, compatibility).

Write the acceptance criteria as Gherkin scenarios before touching code. They are the contract the tests prove and the gates check:
...
One scenario per behavior, including the important failure and edge cases. Non-functional requirements become scenarios when observable, otherwise a short `Constraints:` list under the feature. Keep the scenarios in the plan; do not rewrite them later to fit what was built.

Hand-off: if the request describes existing behavior as wrong, broken, or unexpected, stop and switch to `bug-fixing` before implementing anything. The defect is characterized first; the enhancement follows as a separate change.
...
Plan gate: whenever any of those conditions applies, or the user asks for it, hand the plan to `review-gate` (mode `plan`) before implementing. Fix BLOCKING findings and re-run, two rounds at most. In an interactive run, present the plan and the gate's verdict together and wait; in a non-interactive run, end the turn with both and no code changes. When no condition applies, move directly from plan to implementation.
...
Map each Gherkin scenario to a test, or write one line stating why it cannot be tested at any level below the UI; the diff gate checks this mapping. Do not add tests only to raise coverage.
...
Mandatory. Hand the finished change to `review-gate` (mode `diff`) with the request, the scenarios, the branch or commit range, the stack skill name, and the round number.

- FAIL → fix every BLOCKING and SHOULD FIX finding, re-run your own validation, and re-run the gate with the next round number. Three rounds at most.
- Still FAIL after round 3 → stop, keep the change as is, and list the open findings in the report. Do not keep iterating and do not weaken the criteria to obtain a PASS. If a scenario turns out to be wrong, say so in the report and leave it for a human to decide.
...
## Tests
<scenario → test mapping; untested scenarios with reason> | none (why)

## Review gate
- plan gate: PASS (round n) | skipped (no pause condition) | not run (why)
...
- Never edit the Gherkin scenarios to make a gate pass; a wrong scenario is reported, not silently corrected.
