---
name: bug-fixing
description: Diagnose and fix existing defects in any codebase by reproducing the problem, establishing root cause with evidence, adding regression coverage when practical, and applying the smallest safe correction. Use for crashes, exceptions, incorrect results, regressions, intermittent failures, state and concurrency bugs, UI that does not update, and production incidents, even when the user only says "this doesn't work". Language-agnostic; detects the stack and loads the matching stack skill (dotnet-wpf, blazor) for framework-specific investigation and validation, and hands the root cause and the finished diff to review-gate for independent verification. Do not use as the primary workflow for net-new feature development. As a tie-breaker, when a request mixes a change with a symptom of existing behavior being wrong, this skill takes precedence over feature-development.
---
...
A gate is a separate agent with a fresh context: a subagent, or a new session when subagents are unavailable. Give it only the handoff package defined in the `review-gate` skill (mode, request, scenarios, artifact, stack skill, round). Never run a gate inside your own context and never pass it your transcript. If no independent agent can be started in this environment, write `not run` for that gate in the report; reviewing your own work is not a gate.

## Core rule

Do not jump from symptom to patch. The required chain is:

`Observed behavior → Reproduction → Evidence → Root cause → Regression coverage → Fix → Verification → Independent review`
...
Hand-off: if characterization shows there is no defect and the request is an enhancement, say so and switch to `feature-development`.
...
Then write the expected behavior as Gherkin scenarios. The scenario that currently fails is the defect's contract and the specification for the regression test; add a second scenario for any adjacent behavior the fix must preserve:

```
Scenario: <expected behavior that currently fails>
  Given <state that triggers the defect>
  When <action>
  Then <correct observable result>
```
...
Cause gate: whenever any of those conditions applies, confidence is low, or the user asks for it, hand the root-cause statement to `review-gate` (mode `cause`) before changing code. Fix BLOCKING findings (usually: missing evidence, an unexcluded hypothesis) and re-run, two rounds at most. In an interactive run, present the statement and the verdict together and wait; in a non-interactive run, end the turn with both instead of a speculative patch.

## 5. Add regression protection

Encode the defect in a test before applying the fix when the framework and failure mode make that practical. The test implements the failing scenario from step 1: it should fail against the pre-fix code, pass after the fix, and target the defect's contract rather than incidental implementation. Choose the lowest-level reliable boundary; a service or ViewModel test is preferable to a UI test when it fully proves the failure.
...
Mandatory. Hand the finished change to `review-gate` (mode `diff`) with the request, the scenarios, the root-cause statement, the branch or commit range, the stack skill name, and the round number.

- FAIL → fix every BLOCKING and SHOULD FIX finding, re-run your own verification, and re-run the gate with the next round number. Three rounds at most.
- Still FAIL after round 3 → stop, keep the change as is, and list the open findings in the report. Do not weaken the scenarios or the regression test to obtain a PASS.
...
## Review gate
- cause gate: PASS (round n) | skipped (no stop condition, confidence high) | not run (why)
- diff gate: PASS (round n) | FAIL after 3 rounds — open findings: <list> | not run (why)
...
- Never edit the scenarios or weaken the regression test to make a gate pass; a wrong scenario is reported, not silently corrected.
